...
import { createGift, updateGift, deleteGift, getGifts } from "@/lib/gift-service"
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!giftData.name || !giftData.categoryId || giftData.quantity === undefined) {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    // Validate required fields
    if (!giftData.id || !giftData.name || !giftData.categoryId || giftData.quantity === undefined) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 })
    }

    // Validate quantity
    if (giftData.quantity < giftData.reservedQuantity) {
      return NextResponse.json({ error: "Quantity cannot be less than reserved quantity" }, { status: 400 })
    }
